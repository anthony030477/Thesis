...
def inference(mode,loader,posterior=False,output_files=False):
    ...
    with torch.no_grad():
        for tokens, _, _, querys, answers,all_as,q_a in bar:
            ...
            bar.set_postfix_str(f'acc: {sum(acc_ls)/len(acc_ls):.4f} f1: {sum(f1_ls)/len(f1_ls):.4f} rouge: {sum(rouge_ls)/len(rouge_ls):.4f} bertscore: {sum(bestsore)/len(bestsore):.4f}es:{sum(es_ls)/len(es_ls):.4f}')
